...
const createUser = (request, response) => {
  ...
  pool.query("ALTER TABLE comments ALTER COLUMN created_date SET default CURRENT_DATE;", (error, results) => {
    if (error) {
      throw error
    }
...
const getProject = (request, response) => {
  ...
  pool.query("SELECT projects.project_id, title, status, responsible, TO_CHAR(duedate, 'MM/DD/YYYY') AS duedate, projects.description AS description, comments.description AS commentDescription FROM projects LEFT JOIN comments ON projects.project_id = comments.project_id WHERE projects.project_id =" + id, (error, results) => {
    if (error) {
      throw error
    }
	var projectText = '<br><br>';
	projectText += "<form action='/updateProject' method='post' style='white-space:pre-line;'> <b>TITLE:</b> <input type='text' name='title' id='title' value='" + results.rows[0].title + "' hidden>" + results.rows[0].title + "<br><br><b>STATUS:</b> <select id='statusSQL' name='statusSQL'><option value='Open'>Open</option><option value='In-process'>In-process</option><option value='Closed'>Closed</option></select><br><br><b>DUE DATE:</b> <input type='text' name='duedate' id='duedate' value='" + results.rows[0].duedate + "'hidden>" + results.rows[0].duedate + "<br><br><b>RESPONSIBLE:</b> <input type='text' name='responsible' id='responsible' value='" + results.rows[0].responsible + "' hidden>" + results.rows[0].responsible + "<br><br><b>PROJECT ID:</b> <input type='text' name='project_id' id='project_id' value='" + results.rows[0].project_id + "' hidden>" + results.rows[0].project_id + "<br><br><b>DESCRIPTION:</b> <input type='text' name='description' id=description value='" + results.rows[0].description + "' hidden>" + results.rows[0].description + "<br><br><input type='submit' value='Update Project'></form><hr><form action='/addComment' method='post' id='commentDescription' style='white-space:pre-line;'><label for='commentDescription'>Comment:</label><br><br><textarea style='width:300px; height:300px;' name='commentDescription' id='commentDescription' form='commentDescription' Placeholder='Describe your comment here...'></textarea><input type='text' name='project_id' id='project_id' value='" + results.rows[0].project_id + "' hidden><input type='submit' value='ADD COMMENT'></input></form>";
    results.rows.forEach(element => projectText += "<p><b>COMMENT: </b><br><br>" + element.commentdescription + "</p>");
